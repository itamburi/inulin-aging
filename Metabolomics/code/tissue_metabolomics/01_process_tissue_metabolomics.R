...
# 
here::i_am("code/tissue_metabolomics/01_process_tissue_metabolomics.R")
...
dir.create(here::here("data/processed/tissue_metabolomics/ion_counts"), recursive = TRUE, showWarnings = FALSE)
dir.create(here::here("data/processed/tissue_metabolomics/ion_counts/maven_merger"), recursive = TRUE, showWarnings = FALSE)
...
CD_files <- list.files(here("data/raw/tissue/compound_discoverer"), pattern = "\\.xlsx$", full.names = TRUE)
...
for (CD_file in CD_files) {
  ...
  csv_file = file.path(here("data/raw/tissue/compound_discoverer"), paste0(file_name, ".csv"))   # Set the output .csv file path
...
csv_files = list.files(here("data/raw/tissue/compound_discoverer"), pattern = "\\.csv$", full.names = TRUE)
...
# save all features:
write.csv(tidy_df, here("data/processed/tissue_metabolomics/ion_counts/raw CD observations combined and melted.csv"), row.names = FALSE)
...
write.csv(tidy_df2, here("data/processed/tissue_metabolomics/ion_counts/filtered CD observations combined and melted.csv"), row.names = FALSE)
...
expdata  = read.csv(here("data/processed/tissue_metabolomics/ion_counts/filtered CD observations combined and melted.csv"), fileEncoding = "UTF-8-BOM") %>%
...
mz_filt = cpds2 %>%
  filter(
    !matched.reference.name, # if rt.match == TRUE we keep regardless of mzcloud.conf
    mzcloud.conf < 40
  ) 
...
write.csv(expdata2, here("data/processed/tissue_metabolomics/ion_counts/CD data filtered by mismatches to ref and mz cloud conf.csv"),row.names = FALSE)
...
expdata = read.csv(here("data/processed/tissue_metabolomics/ion_counts/CD data filtered by mismatches to ref and mz cloud conf.csv")) %>%
  mutate(
...
write.csv(expdata6, here("data/processed/tissue_metabolomics/ion_counts/annotated and dereplicated CD obs without repeat isomers.csv"), row.names = FALSE)
write.csv(remove, here("data/processed/tissue_metabolomics/ion_counts/names removed due to tissue RT drift or nonsense name.csv"), row.names = FALSE)
...
expdata = read.csv(here("data/processed/tissue_metabolomics/ion_counts/annotated and dereplicated CD obs without repeat isomers.csv"), fileEncoding = "UTF-8-BOM") %>%
  mutate(
...
write.csv(expdata3, here("data/processed/tissue_metabolomics/ion_counts/annotated and processed CD obs consolidated by top ion mode.csv"), row.names = FALSE)
#

# ========== 6.0 - Merge and Consolidate CD and Maven Observations ==========
# --

mav_dir = "data/raw/tissue/maven"
...
write.csv(mav_obs, here("data/processed/tissue_metabolomics/ion_counts/maven_merger/all maven observations.csv"), row.names = FALSE)
...
write.csv(mav_compounds, here("data/processed/tissue_metabolomics/ion_counts/maven_merger/unique compounds and formulas from maven.csv"), row.names = FALSE)

cd_data = read.csv(
  here("data/processed/tissue_metabolomics/ion_counts/annotated and processed CD obs consolidated by top ion mode.csv"),
...
write.csv(cd_compounds, here("data/processed/tissue_metabolomics/ion_counts/maven_merger/unique compounds and formulas from compound discoverer.csv"), row.names = FALSE)

all_cpds = bind_rows(mav_compounds, cd_compounds) %>%
  arrange(ion.mode, name, formula, source, rt, mz)

write.csv(all_cpds, here("data/processed/tissue_metabolomics/ion_counts/maven_merger/unique maven and cd compounds before consolidation.csv"), row.names = FALSE)
...
write.csv(overlap_audit, here("data/processed/tissue_metabolomics/ion_counts/maven_merger/automatic overlap audit between CD and maven.csv"), row.names = FALSE)
...
expdata = bind_rows(
  cd_data_for_merge %>% select(tissue, raw.file, source, sequence.id, sample, ion.mode, name, formula, mz, rt, ion.count),
  mav_data
) %>%
  arrange(tissue, sample, ion.mode, name, source)

write.csv(expdata, here("data/processed/tissue_metabolomics/ion_counts/annotated and processed CD plus MAV obs.csv"), row.names = FALSE)

# ========== 7.0 - Valine and Median Normalization ==========
# --
expdata = read.csv(here("data/processed/tissue_metabolomics/ion_counts/annotated and processed CD plus MAV obs.csv"))
...
write.csv(expdata5, here("data/processed/tissue_metabolomics/ion_counts/tidy and normalized tissue ion count data.csv"), row.names = FALSE)
...
write.csv(expdata_wide, here("data/processed/tissue_metabolomics/ion_counts/tidy and normalized tissue ion count data wide.csv"), row.names = FALSE)
